...
class TetrisEnv(gym.Env):
    ...
    def _get_reward(self):
        ...
        Tried two types of reward functions.
        
        1. Lines cleared in current turn
            - This is very effective when using Dellacherie's features

        2. Fitness function
            - This reward function uses an explicity featurized fitness 
           function. Useful when the state definition is naive (like 
           taking only field and next piece into consideration)
        Idea source: https://codemyroad.wordpress.com/2013/04/14/
                     tetris-ai-the-near-perfect-player/
        
        Params using for computing score:
        1. Aggregate height
        2. Lines cleared in current turn (self.cleared_current_turn)
        3. Holes in the grid
        4. Bumpiness (the sum of the absolute differences in height 
                      between adjacent columns)
        '''
        
        lines_cleared = self.cleared_current_turn
        # agg_ht = np.sum(self.state.top)
        # holes = 0

        # # Co-effients
        # coeff_agg_ht = -0.51
        # coeff_lines_cleared = 0.76
        # coeff_holes = -0.36
        # coeff_bumpiness = -0.18

        # for i in range(self.n_cols):
        #     for j in range(self.state.top[i]-1, -1, -1):
        #         if self.state.field[j, i]==0:
        #             holes+=1
        # bumpiness = np.linalg.norm(np.diff(self.state.top), ord=1)
        # # print(agg_ht, lines_cleared, holes, bumpiness)

        # score = coeff_agg_ht * agg_ht + coeff_lines_cleared * lines_cleared \
        #         + coeff_holes * holes + coeff_bumpiness * bumpiness
